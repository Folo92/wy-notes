...
TypeScript（简称 TS） 是由 Microsoft 开发的、开源的、基于 JavaScript（简称 JS） 的语言，可以被看作是 **JavaScript 的一个超集**。
...
当前版本 TS 中基础类型共有 14 种：
布尔型、数字、字符串、symbol、数组、元组、枚举、any、unknown、void、null、undefined、never、object。
...
同 JS。symbol 是 ES2015 新引入的一种原始类型的值。它类似于字符串，但是每一个 Symbol 值都是独一无二的，与其他任何值都不相等。

```ts
let x: symbol = Symbol();
let y: symbol = Symbol();
// 变量x和y的类型都是 symbol，且都用Symbol()生成，但它们是不相等的
console.log(x === y); // false
```

<details>
<summary>详解 TS 中的 symbol</summary>

1、symbol 包含所有的 Symbol 值，但是无法表示某个具体的 Symbol 值（比如，5 是一个具体的数值，就用 5 这个字面量来表示，这也是它的值类型。但是 Symbol 值不存在字面量，必须通过变量来引用，所以写不出只包含单个 Symbol 值的那种值类型。）

2、解决上述问题 TS 设计了 symbol 的一个子类型 unique symbol，表示单个的、某个具体的 Symbol 值；因为 unique symbol 是单个的不能修改的，所以只能用 const 声明，不能用 let；

```ts
const x: unique symbol = Symbol(); // 正确
let y: unique symbol = Symobol(); // 报错
```
...
a === b; // 报错：变量a和变量b的类型虽然都是unique symbol，但其实是两个值类型。不同类型的值肯定是不相等的，所以最后一行就报错了
// 由于 Symbol 类似于字符串，可以参考下面的例子来理解
const a: "hello" = "hello";
const b: "world" = "world";
a === b; // 报错:虽然都是字符串，但是不同值(类型)，不能使用严格相等运算符进行比较。
```
...
interface Foo {
  [x]: string; // 正确
  [y]: string; // 错误：变量y当作属性名，但是y的类型是 symbol，不是固定不变的值，导致报错。
}
```
...
// 静态只读属性foo的类型就是unique symbol。注意，这时static和readonly两个限定符缺一不可，这是为了保证这个属性是固定不变的
```

3、类型推断：如果声明时没有给出类型，TS 就会推断某个 Symbol 值变量的类型

- let 声明的，推断类型为 symbol；let 声明的变量被 unique symbol 的变量赋值，则推断为还是 symbol

- const 声明的，推断类型为 unique symbol；但是 const 声明的变量，被赋值 symbol 类型的变量，则推断为 symbol

```ts
let x = Symbol(); // 类型为 symbol
const x = Symbol(); // 类型为 symbol
let y = x; // let命令声明的变量,被unique symbol复制后还是symbol类型
const x = Symbol(); // 类型为 unique symbol
let x = Symbol();
const y = x; // const命令声明的变量，被symbol赋值后，变为symbol
```

[参考链接](https://blog.csdn.net/qq_33911541/article/details/136193344)
...
1、基础属性：必须全部包含，类型必须匹配。

2、可选属性：属性名后加“?”，表示可有可无，如果有，类型必须匹配。

可选属性没有赋值的时候，获取到的是 undefined。使用可选属性的时候注意空值处理，尤其是函数类型的可选属性，要先判断再调用。
...
严格来说，额外属性不建议随便使用（确实需要用的时候除外），我们最好准确把握自己写的数据类型。
...
function getName(n: NameOrResolver): Name {
  //这里的函数返回值是string，因为第一行代码对string进行了别名，所以就可以用Name代替string
  if (typeof n === "string") {
    return n;
  } else {
    return n();
...
### 4.2.4 String Literal Types 字符串字面量类型
...
### 4.2.5 Numeric Literal Types 数值字面量类型
...
### 4.2.6 Type Guards 类型保护

类型保护就是一些表达式，它们会在运行时检查以确保在某个作用域里的类型。

要定义一个类型保护，我们只需要简单地为一个函数的返回值为类型谓词。
...
还可以用 typeof 进行类型保护, TS 本身会把 typeof 识别成类型保护，所以可以直接在代码中使用。
